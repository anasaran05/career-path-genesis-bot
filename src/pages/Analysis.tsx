...
const Analysis = () => {
  ...
  const parsedData = geminiResult && typeof geminiResult === "string" ? {
    ...
    roadmap: ["Complete GCP Certification and Clinical Trials training", "Intern with a CRO (Clinical Research Organization)", "Take an online course in pharmacovigilance tools", "Build a LinkedIn profile tailored to the pharma industry"],
    courses: ["ICH-GCP Certification", "Regulatory Affairs Masterclass", "Pharmacovigilance Workshop"],
    skillGaps: [{
        ...
        <div className="max-w-6xl mx-auto mb-8 py-0">
          ...
          <Card className="bg-white border-slate-200 shadow-lg mb-6">
            ...
            <CardContent>
              ...
              {/* Roadmap Steps */}
              <div className="space-y-4">
                {roadmapSteps.map((step, index) => <div key={step.id} className={`border rounded-lg transition-all duration-300 hover:shadow-md ${completedSteps.has(step.id) ? 'bg-green-50 border-green-200' : 'bg-white border-slate-200'}`}>
                    <div className="p-4">
                      <div className="flex items-start space-x-4">
                        {/* Step Number & Checkbox */}
                        <div className="flex flex-col items-center space-y-2">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${completedSteps.has(step.id) ? 'bg-green-500 text-white' : 'bg-navy-100 text-navy-600'}`}>
                            {completedSteps.has(step.id) ? '✓' : index + 1}
                          </div>
                          <button onClick={() => toggleStepCompletion(step.id)} className="p-1 hover:bg-slate-100 rounded transition-colors">
                            {completedSteps.has(step.id) ? <CheckCircle className="w-5 h-5 text-green-500" /> : <Circle className="w-5 h-5 text-slate-400 hover:text-slate-600" />}
                          </button>
                        </div>
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <h3 className={`font-semibold text-lg ${completedSteps.has(step.id) ? 'text-green-700' : 'text-navy-800'}`}>
                                {step.title}
                              </h3>
                              <p className="text-slate-600 mt-1">{step.description}</p>
                            </div>
                            <button onClick={() => toggleStepExpansion(step.id)} className="ml-4 p-2 hover:bg-slate-100 rounded-full transition-colors">
                              {expandedSteps.has(step.id) ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
                            </button>
                          </div>
                          ...
                          {/* Expanded Content */}
                          {expandedSteps.has(step.id) && <div className="mt-4 p-4 bg-slate-50 rounded-lg animate-fade-in">
                              <p className="text-slate-700 mb-4">{step.details}</p>
                              
                              <div>
                                <h4 className="font-medium text-navy-700 mb-2">Resources & Tools:</h4>
                                <ul className="space-y-1">
                                  {step.resources.map((resource, resourceIndex) => <li key={resourceIndex} className="flex items-center text-sm text-slate-600">
              ...
              {completedSteps.size === roadmapSteps.length && <div className="mt-6 p-6 bg-gradient-to-r from-green-100 to-blue-100 rounded-lg text-center animate-scale-in">
          ...
          {parsedData ? <AnalysisResultCards topRoles={parsedData.topRoles} roadmap={parsedData.roadmap} courses={parsedData.courses} skillGaps={parsedData.skillGaps} onReanalyze={handleReanalyze} /> :
